...
12. feladat (5 pont)
    ...
    Elvárt eredmény:
        ...
        SELECT `Region`,COUNT(Region) AS `orszagok szama`
...
bónusz 2. (4 pont)
    ...
    Elvárt eredmény:
        ...
        Bónusz 2: (nemjó):
        SELECT CountryCode, COUNT(`IsOfficial`= 'F') AS `nem hivatalos`
        FROM `countrylanguage`
        WHERE `IsOfficial`= 'F'
...
bónusz 4. (6 pont)
    ...
    Elvárt eredmény:
        ...
        néhány példa:
            ...
            (SELECT `CountryCode`, MAX(`Percentage`) AS `MaxPercentage`
            FROM `countrylanguage`
            GROUP BY `CountryCode`) AS `legtobben`
            ON `countrylanguage`.`CountryCode` = `legtobben`.`CountryCode`
            AND `countrylanguage`.`Percentage` = `legtobben`.`MaxPercentage`
